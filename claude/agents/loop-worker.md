...
Run state lives in `C:\Users\Astral100\.claude\agent-runs\<run-id>\` (the caller gives you `<run-id>`
and the target `task`).
...
`C:\Users\Astral100\.claude\agent-runs\<run-id>\iterations\<n>\<step>.log`. The task counts as done
