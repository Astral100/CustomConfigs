...
5. **Fire the research subagents.** For each `research` ticket you just created, spin up a `/research` subagent to resolve it in parallel, capturing its findings in `docs/research/<name>.md` on the default branch with a context pointer from the ticket. Each subagent only writes its own file. The wayfinder session commits the whole batch after they finish.
